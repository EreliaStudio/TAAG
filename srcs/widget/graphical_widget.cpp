...
{
	return (spk::Vector2(p_screenPosition - viewport().geometry().size / 2 - viewport().geometry().anchor) / spk::Vector2(_nodeSize));
}

spk::Vector2Int GraphicalWidget::convertWorldToScreenPosition(const spk::Vector2& p_worldPosition)
{
	return (p_worldPosition * _nodeSize + viewport().geometry().size / 2 + viewport().geometry().anchor);
}
